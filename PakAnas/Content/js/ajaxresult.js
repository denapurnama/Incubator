    ...
    if (typeof returnResult.Result != "undefined" && typeof returnResult.ErrMesgs != "undefined") {
        if (returnResult.Result == returnResult.ValueSuccess) {
            if (!isNotShowSuccessMesg) {
                ...
                if (returnResult.ErrMesgs.length > 0) mesg += "<br/>Dengan keterangan :<br/>";

                for (i = 0; i < returnResult.ErrMesgs.length; i++) {
                    mesg += returnResult.ErrMesgs[i] + "<br/>";
                }

                $.bootstrapGrowl(mesg, { type: 'success'
                    , allowdismiss: 'true'
                    , align: 'center'
                    , delay: 10000
                });
            ...
            if (returnResult.ErrMesgs != null) {
                ...
                $.bootstrapGrowl(errMesg, { type: 'danger'
                    , allowdismiss: 'true'
                    , align: 'center'
                    , delay: 10000
                });
...
function popUpProgressShow() {
    $.blockUI({
        message: '</br></br><div style="font-weight: bold;font-size: 110%;font-family: Sans-Serif;">Loading</div><div style="font-weight: bold;font-size: 110%;font-family: Sans-Serif;">Please wait...</div></br></br><img src="' + gRootPath + 'Content/images/LoadingImage.gif" /></br></br>',
        css: {
            padding: 0,
            margin: 0,
            width: '22%',
